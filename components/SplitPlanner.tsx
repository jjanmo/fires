    ...
    <div className="rounded-2xl bg-[#111318] border border-[#1e2128] p-5">
      ...
      <div className="grid grid-cols-3 gap-4 pt-4 border-t border-[#1e2128]">
        <div>
          ...
          <p className="text-base font-semibold text-white tabular-nums">{Math.round(perBuyKRW).toLocaleString()}원</p>
          <p className="text-[11px] text-slate-600 font-mono mt-0.5">${perBuyUSD.toFixed(0)}</p>
        </div>
        <div>
          <p className="text-[11px] text-slate-600 mb-1">
            2<span className="normal-case">σ</span> 매수가 기준
          </p>
          <p className="text-base font-semibold text-green-400 tabular-nums">{qtyAtBuy}주</p>
          <p className="text-[11px] text-slate-600 font-mono mt-0.5">@ ${buyPrice}</p>
        </div>
        <div>
          <p className="text-[11px] text-slate-600 mb-1">
            2<span className="normal-case">σ</span> 매도가 기준
          </p>
          <p className="text-base font-semibold text-blue-400 tabular-nums">{qtyAtSell}주</p>
