...
export default function PriceChart({ history, symbol }: { history: HistoryRow[]; symbol: string }) {
          ...
          options={{
            ...
            plugins: {
              ...
              tooltip: {
                ...
                borderWidth: 1,
                padding: 10,
                callbacks: { label: ctx => `${ctx.dataset.label}: ${formatPrice(ctx.parsed.y ?? 0, symbol)}` },
              },
            },
            scales: {
              x: { ticks: { color: c.ticks, maxTicksLimit: 8, font: { size: 10 } }, grid: { color: c.grid } },
              y: { ticks: { color: c.ticks, font: { size: 10 }, callback: v => currencySymbol(symbol) + v }, grid: { color: c.grid } },
            },
          }}
        />
      </div>
