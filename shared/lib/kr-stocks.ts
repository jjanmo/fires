...
const loadReverse = (): Record<string, string> => {
...
/** 한글 포함 여부 판별 */
export const isKoreanQuery = (query: string): boolean => /[가-힣]/.test(query);
...
export const searchKrStocks = (query: string, limit = 24) => {
  const map = load();
...
export const getKrStockName = (symbol: string): string | null => {
